...
from submissions.submissions_algorithms_v0_5.AlgoPerf_Team_21.external_tuning.shampoo_submission.optimizers.distributed_shampoo_casprpp.utils.shampoo_block_info import BlockInfo
from submissions.submissions_algorithms_v0_5.AlgoPerf_Team_21.external_tuning.shampoo_submission.optimizers.distributed_shampoo_casprpp.utils.shampoo_utils import (
    compress_list,
    get_dtype_size,
)
...
class ShampooPreconditionerList(PreconditionerList):
    ...
    def update_preconditioners(
        ...
        with profiler.record_function(
            ...
            def precondition_along_axis(g, precond, axis):
                input_vocab = "abcdefghijklmnopqrstuvwxy"
                input_str = input_vocab[:len(g.shape)]
                assert axis< len(g.shape), f"axis: {axis}, g.shape: {g.shape}"
                input_letter = input_str[axis]
                output_letter = "z"
    ...
    def compute_root_inverse(self) -> None:
        ...
        with profiler.record_function(
            ...
            for kronecker_factors, root, order in zip(
                self._local_kronecker_factors_list,
                ...
                root_list_debug = []
                idx = 0
                assert order == len(kronecker_factors.factor_matrices), f"order: {order}, len(kronecker_factors.factor_matrices): {len(kronecker_factors.factor_matrices)}"
                ...
                for (
                    ...
                    #for caspr++ root is overriden
                    root = 2*(order-1 if order>1 else 1) if idx != order-1 else 2
                    root_list_debug.append(root)
                    idx += 1
